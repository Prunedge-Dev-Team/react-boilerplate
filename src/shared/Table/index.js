...
const Table = ({
    ...
    return dataSource.map((row, rowIndex) => {
      ...
      return (
        <TableRow hover key={key} onClick={handleRowClick(row, rowIndex)}>
          {columns.map((column) => (
            <TableCell key={column.dataIndex} align={column.align} style={{ width: column.width }}>
  ...
  const renderTablePagination = () => (
    <TablePagination tableParams={tableParams} setTableParams={setTableParams} />
  );
  ...
  const renderTableHeader = () => (
    ...
    >
      <Box display='flex' justifyContent='space-between'>
        <SearchInput searchTerm={tableParams.search} setSearchTerm={handleSearch} />
        {hasFilter && (
          <Box display='flex' justifyContent='space-between'>
            <Button sx={{ marginLeft: '24px' }} size="large" onClick={() => setOpenFilters(true)}>
              <FilterAltIcon sx={{ marginRight: '10px' }} /> Filter
            </Button>
            <Button sx={{ marginLeft: '15px', display: clearFilterBtn ? 'flex' : 'none', }} size="large" onClick={clearFilter}>
        ...
        {!loading ? (
          <TableWrap elevation={1} square>
            {dataSource.length === 0 ? (
              ...
              <MuiTable>
                <TableHead columns={columns} order={order} orderBy={orderBy} onSort={handleSort} />
